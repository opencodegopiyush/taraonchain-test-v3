  ...
  return (
    <group ref={group}>
      ...
      <mesh ref={ringSel} visible={false}>
        ...
        <meshBasicMaterial color="#4d8dff" transparent opacity={0.85} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
      {/* hover ring */}
      <mesh ref={ringHover} visible={false}>
        <ringGeometry args={[0.965, 1.0, 48]} />
        <meshBasicMaterial color="#eceef2" transparent opacity={0.4} side={THREE.DoubleSide} blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
    </group>
  );
}
