...
const AMBER = new THREE.Color("#e3b95c");
const TEAL = new THREE.Color("#e6d9b8");
const BONE = new THREE.Color("#f2ead8");
...
function Dust({ count }: { count: number }) {
  ...
  return (
    <points geometry={geo}>
      <pointsMaterial map={tex} color="#6b6353" size={0.5} transparent opacity={0.32} depthWrite={false} sizeAttenuation />
    </points>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-0" style={{ touchAction: "pan-y" }} aria-hidden>
      ...
      >
        ...
        <fogExp2 attach="fog" args={["#060503", 0.0125]} />
        <ambientLight intensity={0.5} />
        <directionalLight position={[18, 26, 12]} intensity={1.15} color="#f6ecd2" />
        <directionalLight position={[-16, -8, -14]} intensity={0.35} color="#e6d9b8" />
