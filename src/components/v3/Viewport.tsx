...
import { useStore, rigBus } from "@/lib/store";
...
/* ── focus-on-select — the v5 bubble-visibility fix ───────────
   when a bubble is tapped the camera eases over to center it,
   so the compact entity card (bottom 36dvh) can never cover it.
   rides the frozen rig's own refocus command — zero engine edits. */
function FocusOnSelect() {
  const selectedNodeId = useStore((s) => s.selectedNodeId);
  const last = useRef<string | null>(null);
  useEffect(() => {
    if (selectedNodeId && selectedNodeId !== last.current) {
      rigBus.emit({ type: "refocus", nodeId: selectedNodeId, radius: 15 });
    }
    last.current = selectedNodeId;
  }, [selectedNodeId]);
  return null;
}
          ...
          <span className="hidden shrink-0 items-center gap-1 sm:flex" aria-hidden>
            {CASE.chapters.map((c, i) => (
              <span
                ...
                style={{ background: i <= chapter ? "var(--assess)" : "rgba(242,234,216,0.14)" }}
              />
            ))}
          </span>
        </div>
