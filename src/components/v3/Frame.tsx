...
export function Frame({
  ...
  return (
    ...
    >
      {/* corner brackets */}
      <span aria-hidden className="pointer-events-none absolute -top-px -left-px h-3.5 w-3.5 border-t border-l" style={{ borderColor: "rgba(77,141,255,0.55)" }} />
      <span aria-hidden className="pointer-events-none absolute -top-px -right-px h-3.5 w-3.5 border-t border-r" style={{ borderColor: "rgba(77,141,255,0.55)" }} />
      <span aria-hidden className="pointer-events-none absolute -bottom-px -left-px h-3.5 w-3.5 border-b border-l" style={{ borderColor: "rgba(77,141,255,0.55)" }} />
      <span aria-hidden className="pointer-events-none absolute -bottom-px -right-px h-3.5 w-3.5 border-b border-r" style={{ borderColor: "rgba(77,141,255,0.55)" }} />

      {/* header strip */}
      {(label || meta) && (
        <div
          className="relative z-30 flex h-9 shrink-0 items-center justify-between gap-3 border-b px-3"
          style={{ borderColor: "var(--line)", background: "rgba(14, 17, 23, 0.85)" }}
        >
        ...
        {scan && (
          <div aria-hidden className="pointer-events-none absolute inset-0 z-20 overflow-hidden">
            <div
              ...
              style={{
                background:
                  "linear-gradient(90deg, transparent, rgba(62,207,154,0.35) 30%, rgba(77,141,255,0.4) 70%, transparent)",
              }}
            />
          </div>
        )}
      ...
      {footer && (
        <div className="relative z-30 shrink-0 border-t" style={{ borderColor: "var(--line)", background: "rgba(14, 17, 23, 0.85)" }}>
          {footer}
        </div>
      )}
    </div>
