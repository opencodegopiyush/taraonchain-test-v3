...
const PEEK = 172; // px — the collapsed sheet bar (handle + title + transport)
...
export function DossierColumn({ variant = "column" }: { variant?: "sheet" | "column" }) {
  ...
  const snapPx = (s: number) => (s === 0 ? PEEK : s === 1 ? Math.round(window.innerHeight * 0.52) : Math.round(window.innerHeight * 0.86));
  ...
  /* when a bubble is tapped, get out of the way — the entity sheet
     owns the screen; the dossier drops back to the peek bar */
  useEffect(() => {
    if (variant === "sheet" && selectedNodeId) setSnap(0);
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 px-4 pt-0.5 sm:px-5 lg:pt-3">
        ...
        <h2 className="disp mt-1 text-[20px] font-semibold leading-snug" style={{ color: "var(--ink)" }}>
          {ch.title}
        </h2>
      </div>
      ...
      <div
        ...
        style={{ borderColor: "var(--line)", background: "rgba(14, 17, 23, 0.8)" }}
      >
        <div className="flex items-center justify-between gap-3">
