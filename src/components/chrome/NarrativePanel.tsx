          ...
          >
            {CASE.chapters.map((c, i) => (
              <button
                ...
                role="menuitem"
                onClick={() => jump(i)}
                className="flex w-full items-baseline gap-3 border-b px-2.5 py-2.5 text-left last:border-b-0 transition-colors hover:bg-[rgba(232,228,218,0.05)]"
          ...
          <div className="flex flex-1 items-center gap-[5px]">
            {CASE.chapters.map((c, i) => (
              <button
                ...
                style={{ background: i <= chapter ? "var(--assess)" : "rgba(232,228,218,0.12)" }}
