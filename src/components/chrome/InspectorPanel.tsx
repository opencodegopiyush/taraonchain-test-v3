...
export function InspectorPanel({ confined = false }: { confined?: boolean }) {
  ...
  return (
    <>
      {/* mobile focus dimmer — the entity sheet is the screen now;
          nothing competes with the bubble info while it's open */}
      {!confined && (
        <div
          aria-hidden
          onClick={closed}
          className={`fixed inset-0 z-[38] transition-opacity duration-300 lg:hidden ${
            node ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
          style={{ background: "rgba(4,6,9,0.55)", backdropFilter: "blur(2px)", WebkitBackdropFilter: "blur(2px)" }}
        />
      )}
      <aside
        aria-label="entity inspector"
        className={`inspector-sheet t4-spring fixed inset-x-0 bottom-0 z-40 slim-scroll flex h-[72dvh] flex-col overflow-hidden rounded-t-[12px] border-t lg:inset-x-auto lg:right-0 lg:top-0 lg:bottom-0 lg:h-auto lg:w-[368px] lg:rounded-t-none lg:border-t-0 lg:border-l ${
          node ? "is-open" : ""
      ...
      style={{
        ...
        boxShadow: "0 -12px 40px rgba(4,6,9,0.55)",
      ...
      {node && data && (
        <>
          ...
          <div className="shrink-0 px-4">
            <h2 className="disp mt-2 text-[23px] font-semibold leading-tight" style={{ color: "var(--ink)" }}>
              {node.label}
            </h2>
              ...
              <div>
                ...
                {node.attribution && (
                  <div
                    ...
                    style={{ borderColor: "rgba(77,141,255,0.35)", background: "rgba(77,141,255,0.06)" }}
                  >
