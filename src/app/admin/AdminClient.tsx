  ...
  return (
    <main className="min-h-screen" style={{ background: "var(--scene)" }}>
      ...
      <header
        ...
        style={{ background: "rgba(9,10,14,0.94)", backdropFilter: "blur(10px)", borderBottom: "1px solid var(--line)" }}
      >
        <div className="flex items-center gap-3">
      ...
      <div className="mx-auto max-w-[880px] px-5 py-8 lg:px-8">
        ...
        {/* validation errors */}
        {errors.length > 0 && (
          <div className="mt-8 border p-4" style={{ borderColor: "rgba(242,85,85,0.4)", background: "rgba(242,85,85,0.06)" }} role="alert">
...
function Row({ no, children, onRemove }: { no: number; children: React.ReactNode; onRemove?: () => void }) {
  return (
    <div className="relative border p-4" style={{ borderColor: "var(--line)", background: "rgba(236,238,242,0.015)" }}>
