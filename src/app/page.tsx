    ...
    <main className="fixed inset-0 flex flex-col overflow-hidden" style={{ background: "var(--scene)" }}>
      ...
      <div className="relative min-h-0 flex-1 lg:flex lg:flex-row">
        ...
        {/* mobile — the case file as a drag sheet over the trace.
            while an entity is open it slides away: the compact
            entity card owns the bottom edge instead */}
        <div
          className={`absolute inset-x-0 bottom-0 z-30 transition-transform duration-[520ms] lg:hidden ${
            selectedNodeId ? "translate-y-full" : "translate-y-0"
          }`}
          style={{ transitionTimingFunction: "cubic-bezier(0.32, 0.72, 0.24, 1.08)" }}
          aria-hidden={selectedNodeId ? true : undefined}
        >
        ...
        {/* mobile — compact entity card rides above the trace */}
        <InspectorPanel />
      </div>
